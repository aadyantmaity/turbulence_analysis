...
def plot_detailed_turbulence(df, title_prefix, save_dir):
    for start_year in year_chunks:
        ...
        plt.scatter(mod_turbulence['valid'], mod_turbulence['fl'], color=mod_color, label='Moderate Turbulence (MOD)', alpha=0.7)
        plt.scatter(sev_turbulence['valid'], sev_turbulence['fl'], color=sev_color, label='Severe Turbulence (SEV)', alpha=0.7)
        plt.scatter(combined_turbulence['valid'], combined_turbulence['fl'], color=combined_color, label='Moderate-Severe Turbulence (MOD-SEV)', alpha=0.7)
        ...
        grouped_by_date = chunk_df.groupby(chunk_df['valid'].dt.date)

        for date, date_group in grouped_by_date:
            row = date_group.iloc[0]
